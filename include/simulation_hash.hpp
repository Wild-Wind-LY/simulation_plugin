...
namespace simulation {
  ...
  public:
    ...
    std::string hex() {
      ...
      for (uint32_t word : state_) {
        for (int shift = 28; shift >= 0; shift -= 4) out.push_back(digits[(word >> shift) & 0xf]);
      }
      return out;
    }
  ...
  // Cheap (stat-only, no content reads) summary of a directory tree: total file
  // count, total bytes, and the newest mtime seen. Meant as a fast "did anything
  // plausibly change" pre-check before paying for a full sha256_directory() --
  // an unchanged fingerprint doesn't *prove* the content is unchanged, but a
  // changed one always means something was added/removed/modified/touched.
  struct DirectoryFingerprint {
    uint64_t file_count = 0;
    uint64_t total_bytes = 0;
    int64_t max_mtime_ticks = 0;

    bool operator==(const DirectoryFingerprint& other) const {
      return file_count == other.file_count && total_bytes == other.total_bytes
             && max_mtime_ticks == other.max_mtime_ticks;
    }
  };

  inline DirectoryFingerprint directory_fingerprint(const std::filesystem::path& dir) {
    namespace fs = std::filesystem;
    DirectoryFingerprint out;
    ...
    for (auto it = fs::recursive_directory_iterator(
      ...
      ++out.file_count;
      std::error_code ec;
      out.total_bytes += static_cast<uint64_t>(it->file_size(ec));
      const auto mtime = it->last_write_time(ec);
      if (!ec)
        out.max_mtime_ticks = std::max(out.max_mtime_ticks, mtime.time_since_epoch().count());
    }
    return out;
  }
